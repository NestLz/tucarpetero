...
// Más subastas "en vivo" generadas de forma determinística a partir del
// catálogo real, para que /subastas no dependa solo de las 3 fijas de
// arriba. Mismo mecanismo sembrado que priceHistoryFor/offersFor.
function generatedAuctionFor(card) {
  const rand = mulberry32(seedFromId(card.id) ^ 0xa5a5a5a5);
  const market = marketPriceOf(card);
  const conditionLabels = CONDITIONS.map((c) => c.label);
  const sellerInfo = SELLER_TEMPLATE[Math.floor(rand() * SELLER_TEMPLATE.length)];

  const start = Math.max(2, Math.round(market * 0.5));
  const bids = 2 + Math.floor(rand() * 16);
  const progress = Math.min(1, bids / 18) * (0.7 + rand() * 0.5);
  const current = Math.max(start, Math.round(start + (market - start) * progress));
  const minInc = Math.max(1, Math.round(current * 0.02));
  const buyNow = rand() < 0.6 ? Math.round(current * (1.15 + rand() * 0.2)) : null;
  const endsInMin = 20 + Math.floor(rand() * 700);

  return {
    id: `ag-${card.id}`,
    cardId: card.id,
    seller: sellerInfo.seller,
    verified: sellerInfo.verified,
    cond: weightedPick(rand, conditionLabels, CONDITION_WEIGHTS),
    lang: weightedPick(rand, LANGS, LANG_WEIGHTS),
    start,
    current,
...
const CURATED_IDS = new Set(HAND_CURATED_AUCTIONS.map((a) => a.cardId));
const pokemonPicks = POKEMON_CARDS.filter((_, i) => i % 9 === 3 && POKEMON_CARDS[i]);
const yugiohPicks = YUGIOH_CARDS.filter((_, i) => i % 12 === 4);
const AUCTION_CANDIDATES = [...pokemonPicks, ...yugiohPicks].filter(
  (c) => !CURATED_IDS.has(c.id)
);

export const AUCTIONS = [
  ...HAND_CURATED_AUCTIONS,
  ...AUCTION_CANDIDATES.map(generatedAuctionFor),
];
...
export const SELLERS = {
  ...
  BinderSJM: {
    name: "BinderSJM", rep: 4.2, sales: 23, since: "2025", zone: "San Juan de Miraflores · Lima", verified: false,
    binders: [
      { tcg: "Pokémon", count: 41 },
      { tcg: "Yu-Gi-Oh", count: 18 },
    ],
    reviews: [
      { who: "Renzo T.", stars: 4, txt: "Todo bien, la carta tenía algo más de uso del que esperaba pero avisó antes." },
      { who: "Fiorella A.", stars: 5, txt: "Coordinamos rápido por WhatsApp, sin problemas." },
    ],
  },
  DracoLima: {
    name: "DracoLima", rep: 4.6, sales: 64, since: "2024", zone: "Los Olivos · Lima", verified: false,
    binders: [
      { tcg: "Magic", count: 58 },
      { tcg: "Pokémon", count: 22 },
    ],
    reviews: [
      { who: "Gonzalo P.", stars: 5, txt: "Ganó su subasta y cumplió al toque, la dejó sellada en Vortex Miraflores." },
      { who: "Camila R.", stars: 4, txt: "Buena comunicación, coordinamos en Plaza Norte sin drama." },
      { who: "Iván S.", stars: 5, txt: "Sabe bastante de Magic, buenos precios." },
    ],
  },
  CarpetaMiraflores: {
    name: "CarpetaMiraflores", rep: 4.8, sales: 147, since: "2023", zone: "Miraflores · Lima", verified: true,
    binders: [
      { tcg: "Pokémon", count: 96 },
      { tcg: "Yu-Gi-Oh", count: 40 },
      { tcg: "Magic", count: 15 },
    ],
    reviews: [
      { who: "Andrea L.", stars: 5, txt: "Tienda seria, siempre sella en Vortex antes de entregar." },
      { who: "Bruno M.", stars: 5, txt: "La mejor variedad de sets recientes que he visto en Lima." },
      { who: "Ximena C.", stars: 4, txt: "Todo bien, demoró un poco en responder por temporada alta." },
    ],
  },
  PokeAndrés: {
    name: "PokeAndrés", rep: 4.9, sales: 301, since: "2022", zone: "San Borja · Lima", verified: true,
    binders: [
      { tcg: "Pokémon", count: 210 },
      { tcg: "Yu-Gi-Oh", count: 12 },
    ],
    reviews: [
      { who: "Valentina H.", stars: 5, txt: "El más confiable para Pokémon en Lima, siempre entrega en Arena TCG San Borja." },
      { who: "Mateo D.", stars: 5, txt: "Carta idéntica a las fotos, embalaje impecable." },
      { who: "Sofía N.", stars: 5, txt: "Ya es la tercera vez que le compro, cero problemas." },
    ],
  },
};
...
export const CHATS = [
  {
    id: "c1", with: "KensoTCG", verified: true, cardId: "p42", about: "Mega Charizard X ex · S/ 92",
  ...
  {
    id: "c2", with: "DracoLima", verified: false, cardId: "4", about: "Subasta: Ragavan · puja S/ 132",
  ...
  {
    id: "c3", with: "PokeAndrés", verified: true, cardId: null, about: "Pikachu VMAX · S/ 68",
